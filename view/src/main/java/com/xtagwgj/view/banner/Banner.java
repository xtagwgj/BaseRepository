...
import com.xtagwgj.view.banner.listener.OnBannerListener;
import com.xtagwgj.view.banner.loader.BannerImageLoader;
...
public class Banner extends FrameLayout implements OnPageChangeListener {
    ...
    private boolean isIndicatorShow = BannerConfig.IS_INDICATOR;
    ...
    private void handleTypedArray(Context context, AttributeSet attrs) {
        ...
        isIndicatorShow = typedArray.getBoolean(R.styleable.Banner_is_indicator_show, BannerConfig.IS_INDICATOR);
    ...
    public void update(List<BannerBean> imageUrls) {
        ...
        this.bannerBeanList.addAll(imageUrls);
    ...
    public Banner start() {
        setBannerStyleUI();
        setShowViewList(bannerBeanList);
        setData();
        return this;
    }
    ...
    private void setTitleStyleUI() {
        ...
        if (bannerBeanList.size() > 0) {
            bannerTitle.setText(bannerBeanList.get(0).getTitle());
    ...
    private void setBannerStyleUI() {
        int visibility = isIndicatorShow && count > 1 ? View.VISIBLE : View.GONE;
        switch (bannerStyle) {
    ...
    public void setBannerImageLoader(BannerImageLoader bannerImageLoader) {
        this.bannerImageLoader = bannerImageLoader;
    }

    private void setShowViewList(List<BannerBean> bannerBeanList) {
        if (bannerBeanList == null || bannerBeanList.size() <= 0) {
            bannerDefaultImage.setVisibility(VISIBLE);
            Log.e(tag, "The view data set is empty.");
            return;
        }
        ...
        for (int i = 0; i <= count + 1; i++) {
            ...
            if (bannerImageLoader != null) {
                imageView = bannerImageLoader.createView(context, this);
            }
            if (imageView == null) {
            ...
            BannerBean obj = null;
            if (i == 0) {
                obj = bannerBeanList.get(count - 1);
            } else if (i == count + 1) {
                obj = bannerBeanList.get(0);
            } else {
                obj = bannerBeanList.get(i - 1);
            }
            showViews.add(imageView);
            if (bannerImageLoader != null) {
                bannerImageLoader.displayView(context, obj, imageView, i);
            } else {
    ...
    class BannerPagerAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, final int position) {
            container.addView(showViews.get(position));
            View view = showViews.get(position);
            if (listener != null) {
                view.setOnClickListener(new OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        listener.OnBannerClick(bannerBeanList.get(toRealPosition(position)), toRealPosition(position));
                    }
                });
            }
            return view;
    ...
    public void onPageSelected(int position) {
        ...
        switch (bannerStyle) {
            ...
            case BannerConfig.NUM_INDICATOR_TITLE:
                ...
                bannerTitle.setText(bannerBeanList.get(position - 1).getTitle());
                break;
            case BannerConfig.CIRCLE_INDICATOR_TITLE:
                bannerTitle.setText(bannerBeanList.get(position - 1).getTitle());
                break;
